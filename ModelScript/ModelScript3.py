...
def RobotModule(id):
    ...
    f"\t[R{id}Move] !R{id}Lock & x{id}>1 & !obstacleLeftR{id}-> (x{id}'=x{id}-1) & (R{id}Lock' = true); // left \n" + \
    f"\t[R{id}Move] !R{id}Lock & x{id}<grid_size & !obstacleRightR{id}-> (x{id}'=x{id}+1) & (R{id}Lock' = true); // right \n" + \
    f"\t[R{id}Move] !R{id}Lock & y{id}>1 & !obstacleDownR{id}-> (y{id}'=y{id}-1) & (R{id}Lock' = true); // down \n" + \
    f"\t[R{id}Move] !R{id}Lock & y{id}<grid_size & !obstacleUpR{id}-> (y{id}'=y{id}+1) & (R{id}Lock' = true); // up \n\n" + \
    f"\t[R{id}Wait] !R{id}Lock -> (R{id}Lock' = true); // wait\n"

    for i in range(0, args.num_tasks):
        output += f'\n\t[T{i}R{id}Complete] R{id}Lock & T{i}Active=true & x{id}=tx{i} & y{id}=ty{i}-> (R{id}Lock\' = true);'
